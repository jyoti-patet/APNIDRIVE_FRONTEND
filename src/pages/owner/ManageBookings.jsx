...
const changeBookingStatus=async(bookingId,status)=>{
    try{
      const {data}=await axios.post('/api/bookings/change-status',{booking,status})
      if(data.success){
  ...
  return (
     <div className='px-4 pt-10 md:px-10 w-full'>
      <Title title="Manage Bookings" subTitle="Track all customer booking , approve cancel , requests and manage booking satatuses"/>
            ...
            {booking.map((booking,index)=>(
              <tr key={index}  className='border-t border-borderColor text-gray-500'>
                ...
                <td>{booking.status=== 'pending' ?(
                  ...
                  <sapn className={`px-3 py-1 rounded-full text-xs font-semibold ${booking.status==='confirmed'?"bg-green-100 text-green-500":"bg-red-100 text-red-500"}`}>{booking.status}</sapn>
